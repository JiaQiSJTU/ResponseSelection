...
def padded_tensor(
    ...
    if type(items[0])==list:
        ...
        # lens: List[int] = [len(item) for item in [sample for sample in items]]
        ...
        output = torch.LongTensor(batch_size, par_num, t)
        ...
        for i in range(batch_size):
            for j in range(par_num):
                ...
                if left_padded:
                    # place at end
                    output[i,j, t - lens[idx] :] = items[i][j]
                else:
                    # place at beginning
                    output[i,j, :lens[idx]] = items[i][j]
                idx+=1
