...
class History(object):
    ...
    def update_history(self, obs):
        ...
        if self.field in obs and obs[self.field] is not None:
            # print("self.split_on_newln:{}".format(self.split_on_newln))
            # print("self.add_p1_after_newln:{}".format(self.add_p1_after_newln))
            if self.split_on_newln:
                next_texts = obs[self.field].split('\n')
            else:
                # 新加的三行
                if type(obs[self.field])==list:
            ...
            for text in next_texts:
                ...
                # update history vecs
                self._update_vecs(text)
        # print("self.history_raw_string:{}".format(self.history_raw_strings))
        # print("self.history_string:{}".format(self.history_strings))
        # print("self.history_vec:{}".format(self.history_vecs))
    ...
    def get_history_str(self):
        ...
        """
        # print("get_history_str")
        # exit(0)
        # add here
        # if type(self.history_strings)==list:
        #     temp=[]
        #     for text in self.history_strings:
        #         assert len(text) > 0
        #         temp.append(self.delimiter.join(text))
        #     return temp
    ...
    def get_history_vec(self,task=None):
        """
        Return a vectorized version of the history.
        """
        # print("get_history_vec")
        # print("self.size:{}".format(self.size))
        # print("self.history_vecs:{}".format(self.history_vecs))
        # print("self.history_vecs:{}".format(self.history_vecs[:-1]))
        # exit(0)
        # print("self.vec_type:{}".format(self.vec_type))
        ...
        if self.vec_type == 'deque':
            history = []
            temp = deque(maxlen=self.max_len)
...
class TorchAgent(ABC, Agent):
    ...
    def __init__(self, opt: Opt, shared=None):
        ...
        # 新加的
        # self.task = opt['pytorch-teacher-task']
        self.task = 'None'
    ...
    def _set_text_vec(self, obs, history, truncate):
        ...
        """
        # print("torch.agent.py: _set_text_vec")
        # print("obs['partition_mask']:{}".format(obs['partition_mask']))
        # print("obs['text']:{}".format(obs['text']))
        # print("history:{}".format(history))
        # exit()
        if 'text' not in obs:
            return obs
        ...
        if 'text_vec' not in obs:
            ...
            # print("history_string:{}".format(history_string))
            ...
            if history_string:
                # temp = []
                # for string in history_string:
                #     temp.append()
                obs['text_vec'] = history.get_history_vec(self.task)

        # print("obs[text_vec]:{}".format(obs['text_vec']))
        # print("obs[text_vec].type:{}".format(type(obs['text_vec'])))
        # exit(0)
        ...
        if obs.get('text_vec') is not None:
            # if "par" in self.task:
                truncated_vec = []
                for i in range(len(obs['text_vec'])):
                    truncated_vec.append(torch.LongTensor(self._check_truncate(obs['text_vec'][i], truncate, True)))
                obs.force_set('text_vec', truncated_vec)
            # else:
    ...
    def batchify(self, obs_batch, sort=False):
        ...
        # TEXT
        # print("batchify: sort:{}".format(sort))
        xs, x_lens = None, None
        ...
        # partition_mask
        par_m = None
        if any("partition_mask" in ex for ex in exs):
            par_m = [ex.get('partition_mask',None) for ex in exs]

        # print("par_m:{}".format(par_m))
        # print("text_vec:{}".format(xs))
        # exit(0)
        return Batch(
            text_vec=xs,
            text_lengths=x_lens,
    ...
    def load_state_dict(self, state_dict):
        ...
        """
        # print(state_dict['encoder_ctxt.embeddings.weight'][29738:,:])
        # exit(0)
    ...
    # def _untrainable_parameters(self):
    #     return [p.numel() for p in self.model.parameters() if not p.requires_grad]
