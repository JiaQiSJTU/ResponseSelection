...
class TorchRankerAgent(TorchAgent):
    ...
    def add_cmdline_args(cls, argparser):
        ...
        #     type='bool',
        #     default=False,
        #     help='designed for candidates without positive candidates',
        # )
    ...
    def _build_candidates(self, batch, source, mode):
        ...
        # print(mode)
        # exit(0)
        label_vecs = batch.label_vec  # [bsz] list of lists of LongTensors  bs*length
        label_inds = None
        batchsize = (
            batch.text_vec.size(0)
            if batch.text_vec is not None
        ...
        if source == 'batch':
            ...
            cands = batch.labels
            # print("cands:{}".format(cands))
            cand_vecs = label_vecs
            label_inds = label_vecs.new_tensor(range(batchsize))  #待修改，None对应为-1

            for i, cand in enumerate(cands):
                if cand == None:
                    label_inds[i] = -1

            # print("label_inds:{}".format(label_inds))
            # exit(0)
            ...
            if label_vecs is not None:
                # print("label_vecs:{}".format(label_vecs.shape))
                ...
                for i, label_vec in enumerate(label_vecs):
                    ...
                    # print("self._find_match(cand_vecs[i], label_vec_pad):{}".format(self._find_match(cand_vecs[i], label_vec_pad)))
